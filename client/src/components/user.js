...
export default function User({ match }) {
	...
	useEffect(() => {
		axios
			.get(`https://api.github.com/users/${match.params.login}?`)
			.then((response) => {
				...
				axios.get(`https://api.github.com/users/${match.params.login}/repos?per_page=10`).then((response) => {
					const data = response.data;
					dispatch({
						type: GET_REPOS,
						payload: data,
		...
		<div style={{ background: '#337ab7' }}>
			...
			<div className="card" style={{ margin: '10px' }}>
				<div className="profile-details">
					<div className="profile-details-left">
						<img src={userDetails.avatar_url} className="round-img" alt="" />
					...
					<div className="profile-details-right">
						...
						<a href={userDetails.html_url} className="btn btn-primary my-1" target="_blank" rel = "noreferrer">
			...
			<div className="card " style={{ margin: '10px' }}>
				<ul className="followers-container">
					...
					<li className="public-repo">
						<strong>Public Repos:</strong> {userDetails.public_repos}
					</li>
					<li className="gist">
						{' '}
						<strong>Gists:</strong> {userDetails.public_gist}
					</li>
				</ul>
			</div>
			<div>
				...
				{repos.map((repo) => {
					return (
						<ul key={repo.id} className="card" style={{ margin: '10px' }}>
							<li className="repos">
								<a href={repo.svn_url} target="_blank" rel = "noreferrer" style={{ color: 'black' }}>
									{repo.name}
								</a>
							</li>
						</ul>
